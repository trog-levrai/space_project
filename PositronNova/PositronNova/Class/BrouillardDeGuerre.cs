...
namespace PositronNova.Class
{
    class Cell
    {
        ...
        public Cell(Vector2 position, int largeur, int hauteur)
        {
            ...
            rectPickUp = new Rectangle(50, 50, 50, 50); // C'est pour piocher dans la texture
        }
        ...
        public void Update(Cell[,] tab, int i, int j, int nbCellX, int nbCellY)
        {
            for (int z = 0; z < PositronNova.UnitList.Count; z++)
            {
                if (!tab[i, j].isVisited)
                    if ((PositronNova.UnitList[z].Side == Unit.UnitSide.Humain && PositronNova.UnitList[z].champDeVision.Intersects(tab[i, j].rectangle)) || (PositronNova.planeteList[0].champDeVision.Intersects(tab[i, j].rectangle)))
